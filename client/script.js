document.addEventListener("DOMContentLoaded", () => {
      ...
      if (filteredCrafts.length > 0) {
        filteredCrafts.forEach((craft) => {
          ...
          resultItem.innerHTML = `
                        ...
                        <div id='resultNameContainer'><h6 id='resultName'>${
                          craft.craftName
                        }</h6></div>
                        <div id='resultImgContainer'><img class='resultImg' src="${
                          craft.craftImg || ""
                        }" alt="${craft.craftName} " /></div>
                        <p id='tagline'>${craft.tagline}</p>
                        <div id='resultBtnContainer'><button class="view-craft-btn" data-id="${
                          craft._id
                        }">Details</button></div>
                        </div>
                    `;
  ...
  const filterResultsByCraftType = async (craftTypeId) => {
    ...
    try {
      ...
      const filteredCrafts = crafts.filter(
        (craft) => craft.craftType_id === craftTypeId
      );
      resultsContainer.classList.remove("hidden");
      ...
      if (filteredCrafts.length > 0) {
        filteredCrafts.forEach((craft) => {
          ...
          resultItem.innerHTML = `
                        ...
                        <p id='tagline'>${craft.tagline}</p>
                        <div id='result-difficulty'>Difficulty: ${
                          craft.difficulty
                        }</div>
                        <div id='resultBtnContainer'><button class="view-craft-btn" data-id="${
                          craft._id
  ...
  const filterResultsForPremium = async () => {
    ...
    try {
      ...
      // Filter crafts by
      const filteredCrafts = crafts.filter(
        (craft) => craft.premiumMembership === true
      );
      resultsContainer.classList.remove("hidden");
    ...
    if (e.target.classList.contains("view-craft-btn")) {
      ...
      try {
        ...
        document.getElementById(
          "difficulty"
        ).textContent = `Difficulty: ${craft.difficulty}`;
        ...
        craft.craftReviews.forEach((craftReview) => {
          ...
          let starRating = "";
          if (craftReview.rating === 1) {
            starRating = `⭐`;
          } else if (craftReview.rating === 2) {
            starRating = `⭐⭐`;
          } else if (craftReview.rating === 3) {
            starRating = `⭐⭐⭐`;
          } else if (craftReview.rating === 4) {
            starRating = `⭐⭐⭐⭐`;
          } else if (craftReview.rating === 5) {
            starRating = `⭐⭐⭐⭐⭐`;
          }
  ...
  const getUpdatedReviews = async (craftId) => {
    try {
      const response = await axios.get(
        `http://localhost:5500/crafts/${craftId}/reviews`
      );
      const reviewsList = document.getElementById("reviews-list");
      reviewsList.innerHTML = "";
